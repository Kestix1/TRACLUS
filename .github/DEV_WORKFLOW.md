...
**TL;DR:** Run `devtools::document()`, update `NEWS.md`, then push — the rest is automated.
...
| `no-browser-statement` | stray `browser()` calls |
| `no-debug-statement` | stray `debug()` calls |
| `deps-in-desc` | packages used but not listed in `DESCRIPTION` |
| `lintr` | code style violations |

The commit is **blocked** if any hook fails. Fix the issue and re-commit.
...
Forgetting this causes stale `.Rd` files and will create a diff on the next
...
> The `deps-in-desc` pre-commit hook will block the commit if a package is
> used in the code but not yet listed in `DESCRIPTION`.
...
After bumping the version or changing `DESCRIPTION` metadata, regenerate:
...
| Platform checks (r-hub, Valgrind, WinBuilder) | Only needed before a CRAN submission — see [`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md) |
...
Follow the full step-by-step process in [`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md).
